...
class BidirectionalCell():
    ...
    def output(self, H):
        """ calculates outputs for the RNN
            H: np arr (t, m, 2 * h) of concatenated hidden states both directs
              t: number of time steps
              m: batch size for data
              h: dimensionality of hidden states
            H does not include initialized states
            Returns: Y: the outputs
        """
        # these dimensinos not needed
        t, m, h2 = H.shape
        h = h2 / 2
        ...
        # softmax
        Y = np.exp(Y) / np.sum(np.exp(Y), axis=2)[:, :, np.newaxis]
        return Y
