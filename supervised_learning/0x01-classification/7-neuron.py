...
class Neuron:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True,
        ...
        """ trains the neuron
            args:
            X: np.ndarray (nx, m) containing input data
                nx is number input features
                m is number examples
            Y: np.ndarray (1, m) with correct labels for input data
            iterations: positive int number of iterations to train over
            alpha: positive flaot learning rate

            verbose: boolean of whether not not to print into about training
            graph: boolean of whether  to graph info about training one
                 training has completed
            step: int to show when to get costs when using verbose/graph
        ...
        if verbose or graph:
            ...
            if step <= 0 or step > iterations:
                raise ValueError("step must be positive nad <= iterations")
        if graph:
            g_costs = np.array(())
        for i in range(iterations):
            """forward prop to update A, then gradient descent to
               update the weights and biases
            """
            self.forward_prop(X)
            if verbose:
                if i % step is 0:
                    cost_i = self.cost(Y, self.A)
                    print("Cost after {} iterations: {}".format(i, cost_i))
            if graph:
                if i % step is 0:
                    g_costs = np.append(g_costs, self.cost(Y, self.A))
                    # print(g_costs)
            # self.forward_prop(X)
        ...
        if graph:
            g_costs = np.append(g_costs, self.cost(Y, self.A))
            # plt.plot([i for i in range(0, iterations, step)], g_costs, 'b-')
            plt.plot(g_costs, 'b-')
